...
uint32_t USBD_OTG_ISR_Handler (USB_OTG_CORE_HANDLE *pdev)
{
  ...
  if (USB_OTG_IsDeviceMode(pdev)) /* ensure that we are in device mode */
  {
	...
	// 处理连接成功事件
        if (gintr_status.b.conidstschng)  /* 连接状态变化中断 */
        {
            //retval |= DCD_HandleConnect_ISR(pdev);  // 这里可以添加处理连接成功的函数
        }

        // 处理断开事件
        if (gintr_status.b.disconnect) /* 断开中断，设备与主机断开连接 */
        {
            //retval |= DCD_HandleDisconnect_ISR(pdev);  // 这里可以添加处理连接断开的函数
        }
#ifdef VBUS_SENSING_ENABLED
