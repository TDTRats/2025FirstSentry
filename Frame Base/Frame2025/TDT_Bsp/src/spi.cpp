...
/**
 * @ingroup TDT_BSP
 * @defgroup TDT_BSP_SPI SPI
 * @brief 提供了包括硬件SPI配置、发送和接受
 */
...
void Spi::init(void)
{
	...
	SPI_InitStructure.SPI_BaudRatePrescaler = baud;		//定义波特率预分频的值:波特率预分频值为8
    ...
    // 配置GPIOB3 (CLK), GPIOB4 (MISO), GPIOA7 (MOSI)
    gpioInitStructure.GPIO_Pin = GPIO_Pin_3|GPIO_Pin_4;   // PB3复用为SPI1 CLK
    GPIO_Init(GPIOB, &gpioInitStructure);
		
    gpioInitStructure.GPIO_Pin = GPIO_Pin_7;              // PA7复用为SPI1 MOSI
    GPIO_Init(GPIOA, &gpioInitStructure);

    // 配置引脚复用功能
    GPIO_PinAFConfig(GPIOB, GPIO_PinSource3, GPIO_AF_SPI1); // PB3复用为 SPI1 CLK
	...
	else if(spix == SPI2 && initFlag[SPI::spi2] == false)
	{
		...
		Spi::initFlag[SPI::spi2] = true;
    RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE); // 使能GPIOB时钟
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_SPI2, ENABLE);  // 使能SPI2时钟

   
		gpioInitStructure.GPIO_Pin = GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15;  // PB13~15复用功能
    GPIO_Init(GPIOB, &gpioInitStructure);
		
		GPIO_PinAFConfig(GPIOB, GPIO_PinSource13, GPIO_AF_SPI2); // PB13复用为 SPI2 CLK
    GPIO_PinAFConfig(GPIOB, GPIO_PinSource14, GPIO_AF_SPI2); // PB14复用为 SPI2 MISO
    GPIO_PinAFConfig(GPIOB, GPIO_PinSource15, GPIO_AF_SPI2); // PB15复用为 SPI2 MOSI
...
/**
  ...
  * @return 读取到的字节
  */
u8 Spi::toggle_readByte(u8 regAddr)
{
	csOff();
	u8 val;
	readWriteByte(regAddr | 0x80); //Register. MSB 1 is read instruction.
...
/**
  ...
  *	@param data 数据地址
  */
void Spi::toggle_readBytes(u8 regAddr, u8 len, u8* data)
{
	csOff();
	readWriteByte(regAddr | 0x80); //Register. MSB 1 is read instruction.
	for(u8 i=0;i<len;i++)
...
/**
  ...
  * @param data 写入的数据
  */
void Spi::toggle_writeByte(u8 regAddr, u8 data)
{
	csOff();
	readWriteByte(regAddr & 0x7F); //Register. MSB 0 is write instruction.
	readWriteByte(data); //Send Data to write
...
/**
  ...
  *	@param data 数据地址
  */
void Spi::toggle_writeBytes(u8 regAddr, u8 len, u8* data)
{
	csOff();
	readWriteByte(regAddr & 0x7F); //Register. MSB 0 is write instruction.
	for(u8 i=0;i<len;i++)
