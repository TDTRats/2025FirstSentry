...
//使用BMI088陀螺仪需要读取boardImu->AHRS_data.Angle
...
void Bmi088::init(void)
{
  ...
  RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE); // 使能GPIOB时钟
	...
	gpioInitStructure.GPIO_Pin = GPIO_Pin_4;//PB5~7复用功能输出	
	...
	gpioInitStructure.GPIO_Pin = GPIO_Pin_0;//PB5~7复用功能输出	
	...
	if(bmi_OK==0)
	{
		...
		do //直到配置无error为止
		{
			status = BMI088_NO_ERROR ; //每次配置之前重置错误码
			status |= acc_init() ;
			delayMs(2);
			status |= gyro_init() ;
			delayMs(2);
...
void Bmi088::GetOriginData()
{
	...
	//此处不应使用带通滤波或高通滤波，因为转动的数据频率往往是低频，采用这两种滤波器会降低灵敏度
