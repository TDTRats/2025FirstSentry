...
/* -------------- 配置 DMA -----------------------------------------*/
	...
	RCC_APB1PeriphResetCmd(RCC_APB1Periph_USART3, DISABLE); // 停止复位USART3
	...
	GPIO_PinAFConfig(GPIOC, GPIO_PinSource11, GPIO_AF_USART3); // PB11 为 USART3 RX

	/* -------------- USART 配置 ------------------------------------- */
	USART_DeInit(USART3);

	USART_InitStructure.USART_BaudRate = 100000; // 波特率可根据需求调整
	USART_InitStructure.USART_WordLength = USART_WordLength_8b;
	USART_InitStructure.USART_StopBits = USART_StopBits_1;
...
//	/* -------------- DMA 配置 --------------------------------------- */
	...
	DMA_InitStructure.DMA_Channel = DMA_Channel_4; // USART3 Rx 对应的 DMA 通道
	...
	DMA_DoubleBufferModeConfig(DMA1_Stream1, (uint32_t)SBUS_rx_buf[1], DMA_Memory_0);
...
void _RC::run_1000Hz()
{
	...
	if (timeIntervalFrom(updateTime) > 500000) //500ms
	{
		deforceFlag = 1;
		/*离线检测*/
		if (dbusOfflineFlag == 0)//后面置一代表相当于离线跳变
		{
			//清除旧数据
...
void _RC::handleData(volatile const uint8_t *SBUS_buf)
{
	...
	//左边拨轮   范围+-660
	Key.CH[11] = -1*my_deathzoom(((SBUS_buf[16] | (SBUS_buf[17] << 8)) & 0x07ff)-1024,5); //Channel 11
	...
	/***鼠标Z值***/
	Key.CH[12] = ((SBUS_buf[10]) | (SBUS_buf[11] << 8)); //z
	/***鼠标左键***/
	...
	/***鼠标滚轮***/
	Key.wheelCode += Key.CH[12];//积分得鼠标滚轮位置编码,一般均为10的倍数,会丢值,慎用
	...
	/*▲ 跳变检测*/
	//拨杆跳变检测,刚连接上，进入此函数时dbusOfflineFlag仍然为1
	if(dbusOfflineFlag==0)
	{
...
{
	//调用其他任务的遥控器更新函数
	for (int i = 0; i < taskControl.taskNum; i++)
	{
		taskControl.taskList[i]->remoteCtrlUpdate();
	}
...
void _RC::taskSchedule()
{
	...
	if (Key.SW2 == RCS::Down || Key.SW2 == RCS::Lost)
	{
		...
		{ //调用其他任务的脱力回调函数
			for (int i = 0; i < taskControl.taskNum; i++)
			{
				taskControl.taskList[i]->deforceCallBack();
			}
		}
		deforceFlag = 1;
	...
	else
	{
		if (deforceFlag)
		{
			imutask.forceGetOffset= 0;
			//调用其他任务的脱力回调函数
			for (int i = 0; i < taskControl.taskNum; i++)
			{
				taskControl.taskList[i]->deforceCancelCallBack();
			}
		}
		deforceFlag = 0;
		//清除相关隐患变量
